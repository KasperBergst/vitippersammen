...
export default class FeedbackPage{
    ...
    static addTextInputFunctionality(){
        ...
        const minHeight = (contentDiv.clientHeight / 100) * 20; // 20% of content height
        const maxHeight = (contentDiv.clientHeight / 100) * 70; // 70% of content height
    ...
    static getFeedbackContent(){
        return `
        <div id="leftDiv">
            <div class="feedbackDiv darkGrayBackground" style="display: flex; flex-direction: column; align-items: center; width: 50%; height: fit-content; border: 2px solid black; border-radius: 30px; background-color: var(--middleground-color);">
                <div style="width: 95%; height: 20%; display: flex; justify-content: space-between; margin-top: 2%; margin-bottom: 5%;">
                    <button id="feedbackBackButton" style="width: 10%;" class="fancyButtonStyle">Tilbage</button>
                    <h1>Giv feedback</h1>
