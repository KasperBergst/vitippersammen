...
export default class GroupPage{
	 ...
	 static toggleMemberContent(groupId, groupName, tournamentId){
		  const div = document.getElementById("contentMain");

		  if(div.showingMembers){
				div.showingMembers = false;
				this.loadBetContent(groupId, groupName, tournamentId);
		  }
		  else{
				div.showingMembers = true;
				this.loadMemberContent(groupId, groupName, tournamentId);
		  }
	 };

	 static async loadBetContent(groupId, groupName, tournamentId){
		  document.getElementById("contentMain").innerHTML = this.getGroupContent("bets");
		  ...
		  document.getElementById("toggleMemberContent").addEventListener("click", () => {
				this.toggleMemberContent(groupId, groupName, tournamentId);
		  });

		  document.getElementById("groupDisplayDropdownButton").addEventListener("click", () => {
	 ...
	 static loadMemberContent(groupId, groupName, tournamentId){
		  document.getElementById("contentMain").innerHTML = this.getGroupContent("members");
		  
		  document.getElementById("toggleMemberContent").addEventListener("click", () => this.toggleMemberContent(groupId, groupName, tournamentId));
	 ...
	 static getDate(match){
		  ...
		  return `${weekDays[date.getDay()]} ${date.getDate()}/${date.getMonth() + 1} ${date.getHours()}:${prettyPrintMinutes(date.getMinutes())}`;
	 }
	 ...
	 static placeBet(match, groupId, winner){
		  ...
		  BackendApi.placeBet(match, groupId, winner).then(status => {
				if(status === 200){
					 // reset the color and borders of all boxes
					 ...
					 document.getElementById(`matchTh_${match.matchId}_1`).style.borderBottom = "1px solid black";
					 document.getElementById(`matchTh_${match.matchId}_X`).style.borderBottom = "1px solid black";
					 document.getElementById(`matchTh_${match.matchId}_2`).style.borderBottom = "1px solid black";
					 ...
					 document.getElementById(`matchTh_${match.matchId}_${winner}`).style.borderBottom = "var(--bet-choice-border)";

					 // update global bets object
					 this.bets[match.matchId] = winner;
				}
				else if(status === 401){
	 ...
	 static createMatchRow(match, groupId){
		 ...
		 statisticCell.style = "border-bottom:1px solid black; border-left:2px solid black; width: 5%;";
		  ...
		  .then(data => {
				...
				for(let i = 0; i < data.length; i++){
					 ...
					 if(userId === Authenticator.getCurrentUser().userId){ // the current user
						  ...
						  tds = `
								<td style="border-bottom: var(--bet-choice-border)"><b>${rank}</b></td>
								<td style="border-bottom: var(--bet-choice-border)"><b>${name}</b></td>
								<td style="border-bottom: var(--bet-choice-border)"><b>${score}</b></td>
						  `;
					 }
					 else{
						  tds = `
					 ...
					 if(!data[i].active){ // passive members
	 ...
	 static loadInviteToGroup(groupId, groupName, tournamentId){
		  ...
		  leftDiv.innerHTML = `
		  <div style="display: flex; flex-direction: column; align-items: center; justify-content: flex-start; width: 100%; height: 99%; margin-bottom: 1%;">
	 ...
	 static getGroupContent(mode){
				...
				<div style="display: flex; flex-direction: column; align-items: center; width: 100%; height: 100%;">
					 <div style="display: flex; flex-direction: row; align-items: stretch; justify-content: center; width: 100%;">
						  <div style="width: 20%; display: flex; justify-content: center; align-items: center;">
								<button class="fancyButtonStyle" id="toggleMemberContent" style="padding: 2px; height: 50%;">Tilbage</button>
						  </div>    
						  <div id="membersTitle" style="width: 55%;">
								<h1>Stilling</h1>
						  </div>
						  <div style="width: 20%;"></div>
					 </div>
					 <b id="memberCounter" style="width: 90%; text-align: left;"></b>
					 <div class="groupDisplay" style="overflow: auto;">
				...
				<div style="display: flex; flex-direction: column; align-items: center; width: 95%; height: 100%; margin-top: 1%;">
					 <div style="display: flex; flex-direction: row; align-items: stretch; justify-content: space-around; width: 95%; height: 15%;">
						  ...
						  <div id="groupDisplayDropdownDiv" style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 15%; height: 100%; z-index: 5;">
								<button class="fancyButtonStyle" id="groupDisplayDropdownButton" style="width: 100%; height: 50%;"><image src="${FileLoader.getImagePath("more.png")}" style="width: 70%; height: auto;"></image></button>
								<div id="groupDisplayDropdownContent" style="display: none; flex-direction: column; row-gap: 5%; width: 200%; height: 66%; background-color: var(--middleground-color); border: 2px solid black; border-radius: 20px;">
									 <button id="toggleMemberContent" class="fancyButtonStyle" style="word-wrap: break-word; width: 100%; height: 20%;">Stilling</button>
									 <button id="reloadMatchesButton" class="fancyButtonStyle" style="width: 100%; height: 20%;">Opdater</button>
									 <button id="inviteToGroupButton" class="fancyButtonStyle" style="width: 100%; height: 20%;">Invitér</button>
									 <button id="leaveGroupButton" class="fancyButtonStyle" style="width: 100%; height: 20%;background-color: firebrick"><p style="color: white;">Forlad gruppe</p></button>
								</div>
						  </div>
					 </div>
					 <div style="width: 100%; height: 3%; display: flex; align-items: flex-end; justify-content: space-between">
		  ...
		  return mode === "bets" ? betHTML : memberHTML;
	 }
}
